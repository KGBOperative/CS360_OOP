...
public class ScaleneDialog extends JDialog implements ActionListener {
    ...
    public ScaleneDialog(JFrame frame, boolean modal, int x, int y, int R, int R2, int R3, int A) {
        super(frame, modal);
        oldSide = R;
        oldSide2 = R2;
        oldSide3 = R3;
    ...
    private void addTextAndButtons () {
        ...
        sideText = new JTextField(((Integer) oldSide).toString() + " " + 
                                  ((Integer) oldSide2).toString() + " " + 
                                  ((Integer) oldSide3).toString() + " " + 
                                  ((Integer) oldAngle).toString(), 20);
            ...
            try {
                ...
                if (parts.length > 1) {
                    ...
                    if (parts.length > 2) {
                        ...
                        if (parts.length > 3)
                            angle = Integer.parseInt (parts[3]);
                        else
                            angle = oldAngle;
                    }
                    else {
